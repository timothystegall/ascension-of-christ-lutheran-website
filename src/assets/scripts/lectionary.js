...
function getDisplayPropers({ week, lectionary = [], festivals = [] }) {
  ...
  const primary = lectionary.length === 0 ? festivals : (festivals.length === 0 ? lectionary : (festFirst ? festivals : lectionary));
  const secondary = lectionary.length > 0 && festivals.length > 0 ? (festFirst ? lectionary : festivals) : [];
...
const loader = new KeyLoader({ lectionary: lectionary1yr, festivals, daily, commemorations });
...
function isToday(luxonDate) {
  return luxonDate.day === today.getDate()
...
function getSundayReadings(day) {
  ...
  return {
    ...
    color: findColor(...displayPropers, day.sunday?.propers?.lectionary) ?? null,
  };
}
...
function render(year, month) {
  ...
  for (const week of grid) {
    ...
    for (const day of week) {
      ...
      const colorClass = readings?.color ? ` lect-color-${readings.color.toLowerCase().replace(/\s+/g, "-")}` : "";

      const bgLink = (ref) => `https://www.biblegateway.com/passage/?search=${encodeURIComponent(ref)}&version=NKJV`;
  ...
  const colorKey = [
    ...
    { cls: "white",  label: "White",   desc: "Christmas, Easter & feasts of our Lord" },
    ...
    <div class="lect-color-key">
      ...
      <div class="lect-key-section">
        ...
        <ul class="lect-color-key-list">
          ${readingAbbrevs.map(({ abbr, meaning }) => `
